...
def setup_commands(bot: OpenClawBot):
    ...
    @app_commands.describe(niche="The niche to build (e.g. 'ADHD planner' or 'Gen Z finance tracker')")
    ...
    async def listing_cmd(interaction: discord.Interaction, product: str):
        ...
        if research:
            ...
            if listing and ch:
                embed = discord.Embed(title=f"🏪 Listing: {product}", color=0x00f5a0)
                title = listing.get('etsy_title', '')
                if title:
                    embed.add_field(name="Etsy Title", value=f"```{title[:950]}```", inline=False)
    ...
    async def products_cmd(interaction: discord.Interaction, niche: str):
        await interaction.response.send_message(f"🛒 Generating lineup for: **{niche}**...")
        ch = await bot._get("build-pipeline")
    ...
    @app_commands.describe(action="add or report", product="Product name", amount="Amount in USD")
    async def revenue_cmd(interaction: discord.Interaction,
